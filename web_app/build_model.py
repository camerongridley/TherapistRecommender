...
class TextClassifier(object):
    ...
    def fit(self, X, y):
        ...
        """
        X_tranform = self._vectorizer.fit_transform(X)
        return self._classifier.fit(X_tranform,y)

    def predict_proba(self, X):
        """Make probability predictions on new data."""
        X_tranform = self._vectorizer.transform(X)
        return self._classifier.predict_proba(X_tranform)

    def predict(self, X):
        """Make predictions on new data."""
        X_tranform = self._vectorizer.transform(X)
        return self._classifier.predict(X_tranform)

    def score(self, X, y):
        """Return a classification accuracy score on new data."""
        X_tranform = self._vectorizer.transform(X)
        return self._classifier.score(X_tranform, y)
