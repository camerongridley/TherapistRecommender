...
Not sure what to do.''' 
    ...
    state = 'California'
    n_recs = 5
    # dominant_topic_id, loadings = nmf_recommender.classify_new_text(model, vectorizer, depression_text)
    # print(f'depression_text - dominant topic: {nmf_recommender.get_topic_names([dominant_topic_id])} -- loadings: {loadings}')
    # recs = nmf_recommender.make_recommendations(loadings, df_therapist_topics, state, n_recs)
    # print(f'\nRecs:\n{recs}\n')
    # nmf_recommender.write_html_file(recs[['first_name', 'last_name', 'website', 'phone']].to_html(), 'img/testing/recs.html')

    # dominant_topic_id, loadings = nmf_recommender.classify_new_text(model, vectorizer, ptsd_text)
    # print(f'ptsd_text - dominant topic: {nmf_recommender.get_topic_names([dominant_topic_id])} -- loadings: {loadings}')
    # print(f'\nRecs:\n{nmf_recommender.make_recommendations(loadings, df_therapist_topics, state, n_recs)}\n')

    # dominant_topic_id, loadings = nmf_recommender.classify_new_text(model, vectorizer, addiction_text)
    # print(f'addiction_text - dominant topic: {nmf_recommender.get_topic_names([dominant_topic_id])} -- loadings: {loadings}')
    # print(f'\nRecs:\n{nmf_recommender.make_recommendations(loadings, df_therapist_topics, state, n_recs)}\n')

    # dominant_topic_id, loadings = nmf_recommender.classify_new_text(model, vectorizer, marriage_text)
    # print(f'marriage_text - dominant topic: {nmf_recommender.get_topic_names([dominant_topic_id])} -- loadings: {loadings}')
    # print(f'\nRecs:\n{nmf_recommender.make_recommendations(loadings, df_therapist_topics, state, n_recs)}\n')
